{{-- Shared fields for the page-video add/edit forms.
     Expects:
       $video     — PageVideo (possibly unsaved)
       $mode      — 'create' | 'edit'
     create mode also gets:  $pages (key => label), $pageKey (old value), $taken (keys with a row)
     edit mode also gets:    $pageKey, $pageLabel, $default (config entry array)
--}}
    ...
    <div class="space-y-5 lg:col-span-2">
        <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            ...
            <div class="space-y-5">
                    ...
                    @else
                        <select id="page_key" name="page_key" required
                            ...
                            @foreach ($pages as $key => $label)
                                <option value="{{ $key }}" @selected(old('page_key', $pageKey) === $key)>
                                    {{ $label }}@if (in_array($key, $taken, true)) — already has a custom video @endif
                                </option>
                            @endforeach
                        </select>
                        <p class="mt-1 text-xs text-gray-400">Choosing a page that already has a custom video will replace it.</p>
                    @endif
                </div>

                <div>
                    <label for="video_url" class="mb-1.5 block text-sm font-semibold text-navy-dark">YouTube link</label>
                    <input id="video_url" name="video_url" type="text"
                           value="{{ old('video_url', \Illuminate\Support\Str::startsWith($video->video_url ?? '', ['http://', 'https://']) ? $video->video_url : '') }}"
                    ...
                    <p class="mt-1 text-xs text-gray-400">
                        Paste a YouTube link. Vimeo, Facebook and direct MP4 links also work.
                        {{ $video->exists ? 'Leave empty to keep the current video.' : '' }}
                    </p>
                </div>
                ...
                <div>
                    <label for="title" class="mb-1.5 block text-sm font-semibold text-navy-dark">Caption <span class="text-gray-400">(optional)</span></label>
                    <input id="title" name="title" type="text" value="{{ old('title', $video->title) }}"
                           placeholder="{{ $isEdit ? ($default['title'] ?? 'Watch') : 'Leave blank to keep the page default' }}"
                           class="h-11 w-full rounded-lg border border-gray-300 bg-white px-3.5 text-sm text-navy-dark outline-none transition focus:border-brand focus:ring-2 focus:ring-brand/30">
                    <p class="mt-1 text-xs text-gray-400">Small label shown on the video card. Leave blank to keep the page’s default caption.</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-5">
        <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-gray-400">When hidden, the page falls back to its default video.</p>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            <div class="flex flex-col gap-3">
                ...
                <a href="{{ route('admin.page-videos.index') }}" class="flex h-11 w-full items-center justify-center rounded-lg border border-gray-200 text-sm font-semibold text-navy transition hover:bg-gray-50">
                    Cancel
                </a>
            </div>
        </div>
