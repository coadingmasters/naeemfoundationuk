...
@section('title', 'Hajj Videos')
@section('heading', 'Hajj Videos')
@section('subheading', 'These videos appear in the "Recent Hajj Reviews" gallery on the Hajj 2027 page.')

@section('actions')
    <a href="{{ route('admin.hajj-videos.create') }}"
...
@section('content')
    @if ($videos->isEmpty())
        <div class="rounded-2xl border border-dashed border-gray-300 bg-white p-12 text-center">
            ...
            <p class="mt-1 text-sm text-gray-500">Add a YouTube/Vimeo link or upload a video to populate the Hajj reviews gallery.</p>
            <a href="{{ route('admin.hajj-videos.create') }}" class="mt-5 inline-flex items-center gap-2 rounded-lg bg-brand px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-brand-dark">
                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 5v14M5 12h14" stroke-linecap="round"/></svg>
                Add a Video
            </a>
        </div>
                            ...
                            <tr class="transition hover:bg-gray-50/70">
                                ...
                                <td class="px-5 py-3">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('admin.hajj-videos.edit', $video) }}"
                                        ...
                                        <form method="POST" action="{{ route('admin.hajj-videos.destroy', $video) }}"
                                              >
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" data-admin-delete data-label="{{ \Illuminate\Support\Str::limit($video->title, 40) }}"
