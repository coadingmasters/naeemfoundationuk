{{-- Shared fields for create/edit. Expects $cause (Cause, possibly unsaved). --}}
<div class="grid gap-6 lg:grid-cols-3">
    {{-- Main fields --}}
    <div class="space-y-5 lg:col-span-2">
        <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            <h3 class="mb-4 text-sm font-semibold text-navy-dark">Cause content</h3>

            <div class="space-y-5">
                <div>
                    <label for="title" class="mb-1.5 block text-sm font-semibold text-navy-dark">Title <span class="text-red-500">*</span></label>
                    <input id="title" name="title" type="text" value="{{ old('title', $cause->title) }}" required
                           placeholder="Give Zakat"
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="2" required maxlength="255"
                              placeholder="Health is a basic human right in rural areas…"
                              class="w-full rounded-lg border border-gray-300 bg-white px-3.5 py-2.5 text-sm text-navy-dark outline-none transition focus:border-brand focus:ring-2 focus:ring-brand/30">{{ old('description', $cause->description) }}</textarea>
                ...
                <div>
                    ...
                    <input id="link" name="link" type="text" value="{{ old('link', $cause->link) }}"
                           placeholder="https://… or /donate"
                           class="h-11 w-full rounded-lg border border-gray-300 bg-white px-3.5 text-sm text-navy-dark outline-none transition focus:border-brand focus:ring-2 focus:ring-brand/30">
                    <p class="mt-1 text-xs text-gray-400">Where the "Donate" button links. Leave empty for none.</p>
                </div>
            </div>
        </div>
                ...
                <div class="shrink-0">
                    <div class="relative h-28 w-44 overflow-hidden rounded-lg bg-gray-100 ring-1 ring-gray-200">
                        <img data-image-preview
                             src="{{ $cause->image ? asset($cause->image) : '' }}"
                             alt="Preview"
                             class="h-full w-full object-cover {{ $cause->image ? '' : 'hidden' }}">
                        <span data-image-placeholder class="absolute inset-0 grid place-items-center text-gray-300 {{ $cause->image ? 'hidden' : '' }}">
    ...
    <div class="space-y-5">
        <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input id="sort_order" name="sort_order" type="number" min="0" max="9999" value="{{ old('sort_order', $cause->sort_order) }}" placeholder="Auto"
                           class="h-11 w-full rounded-lg border border-gray-300 bg-white px-3.5 text-sm text-navy-dark outline-none transition focus:border-brand focus:ring-2 focus:ring-brand/30">
                    <p class="mt-1 text-xs text-gray-400">Leave empty to place it automatically after the last one (a gap from a deleted cause is filled first). Lower numbers show first.</p>
                </div>
                ...
                <div>
                    ...
                    <label class="flex cursor-pointer items-center justify-between rounded-lg border border-gray-200 px-3.5 py-3">
                        ...
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $cause->is_active ?? true) ? 'checked' : '' }}
        ...
        <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            <div class="flex flex-col gap-3">
                ...
                <a href="{{ route('admin.causes.index') }}" class="flex h-11 w-full items-center justify-center rounded-lg border border-gray-200 text-sm font-semibold text-navy transition hover:bg-gray-50">
                    Cancel
                </a>
            </div>
        </div>
