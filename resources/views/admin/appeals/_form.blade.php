{{-- Shared fields for create/edit. Expects $appeal (Appeal, possibly unsaved). --}}
<div class="grid gap-6 lg:grid-cols-3">
    {{-- Main fields --}}
    <div class="space-y-5 lg:col-span-2">
        <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            <h3 class="mb-4 text-sm font-semibold text-navy-dark">Appeal content</h3>

            <div class="space-y-5">
                <div>
                    <label for="title" class="mb-1.5 block text-sm font-semibold text-navy-dark">Title <span class="text-red-500">*</span></label>
                    <input id="title" name="title" type="text" value="{{ old('title', $appeal->title) }}" required
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" required maxlength="500"
                              placeholder="Sadaqah: The Power of Giving. Have you ever felt the true…"
                              class="w-full rounded-lg border border-gray-300 bg-white px-3.5 py-2.5 text-sm text-navy-dark outline-none transition focus:border-brand focus:ring-2 focus:ring-brand/30">{{ old('description', $appeal->description) }}</textarea>
                    <p class="mt-1 text-xs text-gray-400">Shown under the title on the homepage card (max 500 characters).</p>
                </div>

                <div>
                    <label for="link" class="mb-1.5 block text-sm font-semibold text-navy-dark">Link <span class="text-gray-400">(optional)</span></label>
                ...
                <div class="shrink-0">
                    <div class="relative h-28 w-44 overflow-hidden rounded-lg bg-gray-100 ring-1 ring-gray-200">
                        <img data-image-preview
                             src="{{ $appeal->image ? asset($appeal->image) : '' }}"
                             alt="Preview"
                             class="h-full w-full object-cover {{ $appeal->image ? '' : 'hidden' }}">
                        <span data-image-placeholder class="absolute inset-0 grid place-items-center text-gray-300 {{ $appeal->image ? 'hidden' : '' }}">
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <label class="flex cursor-pointer items-center justify-between rounded-lg border border-gray-200 px-3.5 py-3">
                        ...
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $appeal->is_active ?? true) ? 'checked' : '' }}
        ...
        <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            <div class="flex flex-col gap-3">
                <button type="submit" class="flex h-11 w-full items-center justify-center gap-2 rounded-lg bg-brand text-sm font-semibold text-white transition hover:bg-brand-dark">
                    ...
                    {{ $appeal->exists ? 'Save Changes' : 'Create Appeal' }}
                </button>
                <a href="{{ route('admin.appeals.index') }}" class="flex h-11 w-full items-center justify-center rounded-lg border border-gray-200 text-sm font-semibold text-navy transition hover:bg-gray-50">
                    Cancel
                </a>
            </div>
        </div>
