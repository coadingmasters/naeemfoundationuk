    ...
    <div class="nf-xscroll -mx-3" aria-label="Children waiting for a sponsor">
        <div class="nf-xscroll__track" style="animation-duration: {{ $duration }}s">
            @foreach ($loopOrphans as $i => $orphan)
                ...
                <div class="nf-xscroll__item w-[230px] px-3 sm:w-[250px]" @if ($isClone) aria-hidden="true" @endif>
                        ...
                        <div class="relative aspect-[4/5] overflow-hidden bg-gradient-to-b from-cream to-cream/50">
                            ...
                            @else
                                <div class="grid h-full place-items-center">
                                    <svg class="h-16 w-16 text-navy/15" viewBox="0 0 24 24" fill="currentColor"><path d="M12 12a5 5 0 1 0-5-5 5 5 0 0 0 5 5zm0 2c-4 0-8 2-8 5v1h16v-1c0-3-4-5-8-5z"/></svg>
                                </div>
                            @endif
                            ...
                            @if ($orphan->grade)
                                <span class="absolute left-3 top-3 rounded-full bg-white/90 px-2.5 py-1 text-[10px] font-bold text-brand shadow-sm backdrop-blur">{{ $orphan->grade }}</span>
                            @endif
                        </div>

                        {{-- Details --}}
                        <div class="flex flex-1 flex-col p-4">
                            <h3 class="text-sm font-bold text-navy-dark transition-colors group-hover:text-brand">{{ $orphan->name }}</h3>
                            @if ($orphan->location)
                                <p class="mt-1 inline-flex items-center gap-1.5 text-[11px] text-gray-500">
                                    <svg class="h-3 w-3 shrink-0 text-brand" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 21s-7-5.5-7-11a7 7 0 1 1 14 0c0 5.5-7 11-7 11z" stroke-linecap="round" stroke-linejoin="round"/><circle cx="12" cy="10" r="2.4"/></svg>
                                    {{ $orphan->location }}
                                </p>
                            @endif

                            <span class="btn-brand mt-3 w-full justify-center py-2 text-xs">
                                View &amp; Sponsor
