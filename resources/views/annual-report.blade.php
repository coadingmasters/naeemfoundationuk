            ...
            @else
                <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($reports as $i => $report)
                        <article class="nf-reveal group flex flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
                                 style="transition-delay: {{ $i * 90 }}ms">

                            {{-- Cover --}}
                            <div class="relative h-48 overflow-hidden bg-navy">
                                @if ($report->cover_url)
                                ...
                                @else
                                    <div class="grid h-full w-full place-items-center bg-gradient-to-br from-navy to-navy-dark text-white">
                                        <svg class="h-14 w-14 opacity-70" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4"><path d="M14 3H7a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V8z" stroke-linejoin="round"/><path d="M14 3v5h5M9 13h6M9 17h4" stroke-linecap="round" stroke-linejoin="round"/></svg>
                                    </div>
                                @endif
                            ...
                            <div class="flex flex-1 flex-col p-5">
                                <h2 class="text-base font-bold text-navy-dark">{{ $report->title }}</h2>

                                @if ($report->summary)
                                    <p class="mt-2 flex-1 text-sm leading-relaxed text-gray-600">{{ $report->summary }}</p>
                                @else
                                ...
                                <div class="mt-5 flex gap-2">
                                    <a href="{{ $report->file_url }}" target="_blank" rel="noopener"
                                       class="btn-brand flex-1 justify-center py-2.5 text-xs">
                                    ...
                                    <a href="{{ $report->file_url }}" target="_blank" rel="noopener"
                                       ...
                                       class="grid w-11 place-items-center rounded-md border border-navy/20 text-navy transition-colors hover:bg-navy hover:text-white">
